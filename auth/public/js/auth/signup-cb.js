...
function setEventHandlers(){
    ...
    $proceedButton.on('click', getNextSignupPage); //all

    let $subPlanButtons = $('th[data-sub-plan], td[data-sub-plan]');
    $subPlanButtons.on('click', selectSubPlan); // chooseplan

    let $creditInfoForm = $('.signup-step-creditoption .signup-form');
    $creditInfoForm.on('submit', submitCreditInfo); // creditoption

    let $termsCheckboxLabel = $('.signup-step-creditoption .signup-tou-checkbox-label');
    $termsCheckboxLabel.on('customCheck:', clearWarningText); // creditoption

    let $changePlanLink = $('#signup-change-plan-link');
    $changePlanLink.on('click', getChoosePlanPage); // creditoption

    // regform & creditoption
