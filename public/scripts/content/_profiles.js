...
function startPageLoad(stateElements, levelSelector, fetchType, setHistory){
...
function completePageLoad(args, setHistory){
    ...
    nonRepeatedScripts.forEach(element => {
        ...
        let $script = $('<script>').attr('src', src).attr('type', type);
...
function showPageInHistory(e){
    ...
    startPageLoad(stateElements, '#lowlevel-container', 'state');
}
...
async function showAddProfile(e){
    try {
        ...
        startPageLoad(stateElements, '#lowlevel-container', 'ajax', setHistory);
    } catch (jqXHR){
